...
export default async function CommitmentDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl px-6 py-10">
      <div className="mb-10 flex items-center justify-between border-b border-[var(--line)] pb-6">
        <div>
          <p className="text-sm text-[var(--muted)]">Commitment #{commitment.id}</p>
          <h1 className="mt-2 text-4xl font-semibold text-[var(--ink)]">{commitment.description}</h1>
        </div>
        <Link href="/create">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-5">
              <p className="text-sm text-[var(--muted)]">Proof</p>
              <a href={commitment.proof} className="mt-2 block text-sm text-[var(--ink)] underline" target="_blank" rel="noreferrer">
                {commitment.proof}
              </a>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="rounded-3xl border border-[var(--line)] bg-[var(--surface-2)] px-4 py-5">
              <p className="text-sm text-[var(--muted)]">Countdown</p>
              <p className="mt-2 text-3xl font-semibold text-[var(--ink)]">23h 14m</p>
            </div>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between rounded-2xl border border-[var(--line)] bg-[var(--surface)] px-4 py-3 text-sm">
